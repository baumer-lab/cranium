...
rotation_angle.tbl_brain <- function(data, type = "wildtype", threshold.n=0.9){
 # ro_tidy_brain <- data%>%
 #   tidy(type, threshold = threshold.n)%>%
  #  reorient()
...
#  vertex.x <- -x.coeff/(2*quad.coeff)
 ...
 # vertex <- c(vertex.x, vertex.y)
  ...
  return(rotation_angle_degree)
}

data <- ro_tidy_brain_yt_24
rotation_angle.brain(ro_tidy_brain_yt_24, threshold.n = 0.8)
```
...
path = "Sample/YouToo"
file_names <- list.files(path, pattern=".h5", full.names=TRUE)
brain_tidy <- read_h5(file_names[1])%>%
  tidy(threshold=0.6)%>%
  reorient()


plot2d(brain,title="before PCA")

plot2d(data,title="before error B correction")
c.data <- correct_errorB.tbl_brain(data, threshold.n = 0.6)
plot2d(c.data, title="after error B correction")
```
