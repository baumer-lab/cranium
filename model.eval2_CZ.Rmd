...
m <- list()
for (i in (1: max(threshold_yt_all_final$Sample_id))){
  m <-  threshold_yt_all_final%>%
  filter(Sample_id==i)%>%
  filter(adj.r.squared == max(adj.r.squared))%>%
...
#number of signals for 0.8 threshold
mean(threshold_yt_all_0.8$num.signal)


#One Sample 
path = "Sample/YouToo"
file_names <- list.files(path, pattern=".h5", full.names=TRUE)
summary<-file_names[5]%>%
  read_h5()%>%
  qmodel.brain(threshold.n = 0.8)
