...
threshold_wt <- read_csv("visualization_results/threshold_wt_all_final.csv")%>%
  select(-X1)
...
ggplot(data =threshold_yt, aes(x = threshold, y = adj.r.squared)) +
  geom_boxplot()  + 
  labs(title = "Optimal threshold for Mutatnt (You Too) type")
```
...
temp_wt <- threshold_wt %>% 
  filter(threshold %in% c(0.6,0.62,0.64,0.66,0.7,0.75, 0.8,0.81,0.82,0.83, 0.84,0.85,0.86, 0.87,0.88,0.89, 0.9,0.91,0.92)) %>% 
