...
#Read file
file_names.1 <-  file_names [1]  #select files
...
#Tidy
tidy_brain_1 <- brain_ls_wt.1[[1]]%>%
  tidy()
#Reorient
ro_tidy_brain_1<- reorient(tidy_brain_1)

rgl::plot3d(ro_tidy_brain_1)
...
ggplot( data = tidy_brain_1, aes(x = Freq)) + geom_histogram(binwidth = 0.04) + labs(title = "Wild type Distribution of signal frequency") + 
...
#Tidy
tidy_brain_youtoo_1 <- brain_ls_youtoo.1[[1]]%>%
  tidy(threshold = 0)
#Reorient
ro_tidy_brain_youtoo_1<- reorient(tidy_brain_youtoo_1)

#rgl::plot3d(ro_tidy_brain_1)


ggplot( data = tidy_brain_youtoo_1, aes(x = Freq)) + geom_histogram(binwidth = 0.04) + labs(title = "You Too Distribution of signal frequency-one sample") + 
  theme(axis.text.x = element_text(size=18),
          axis.text.y = element_text(size=18),
          axis.title = element_text(size = 18, face= "bold"))
...
ro_brain_ls_wt<- list()
#Read file
file_names.1 <-  file_names [1:3]  #select files
for (i in (1:length(file_names.1))) {
  brain_ls_wt.1[[i]] <- file_names.1[i] %>%
  read_h5()%>%
  tidy()
  ro_brain_ls_wt[[i]] <- reorient(brain_ls_wt.1[[i]])
  name_wt.1[i] <- sub("_Probabilities.h5", "", file_names.1[i])
}

plotlist_wt<- list()
for (i in (1:length(ro_brain_ls_wt))){
  plotlist_wt[[i]] <- plot2d(ro_brain_ls_wt[[i]], title=file_names.1[[i]])
}
```
...
```{r eval=FALSE, include=FALSE}
  ...
  brain_ls_youtoo.1 <- list()  #Brain data files are stored in a list: brain_ls_wt
  name_youtoo.1 <- list()  #Name of the files are stored in a list: name_wt
  ro_brain_ls_youtoo<- list()
  #Read file
  file_names.1 <- file_names[30]  #select files 
  for (i in (1:length(file_names.1))) {
    brain_ls_youtoo.1[[i]] <- file_names.1[i] %>%
    read_h5()%>%
    tidy(type="youtoo", threshold.n = 0.7)
    ro_brain_ls_youtoo[[i]] <- reorient(brain_ls_youtoo.1[[i]])
    name_youtoo.1[i] <- sub("_Probabilities.h5", "", file_names.1[i])
  }
  
  plotlist_youtoo<- list()
  for (i in (1:length(ro_brain_ls_youtoo))){
    plotlist_youtoo[[i]] <- plot2d(ro_brain_ls_youtoo[[i]], title=file_names.1[[i]])
  }


sum_tb <- file_names.1 %>%
  read_h5()%>%
  qmodel.brain(threshold.n=0.9)
sum_tb 
```
...
#Type 1 : low number of signals, high r^2
AT #19
```{r}
type1 <- qmodel_sum_tb_youtoo %>%
  filter(num.signal <400)%>%
  filter(adj.r.squared>0.8)
```

#Type 2 : low number of signals, low r^2
AT #335
```{r}
type2 <- qmodel_sum_tb_youtoo %>%
  filter(num.signal <400)%>%
