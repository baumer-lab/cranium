...
brain_ls_wt <- list()  #Brain data files are stored in a list: brain_ls_wt
name_wt <- list()  #Name of the files are stored in a list: name_wt

for (i in (1:length(file_names))) {
  brain_ls_wt[[i]] <- file_names[i] %>%
  read_h5()
  name_wt[i] <- sub("_Probabilities.h5", "",file_names[i])
}
```
...
name_youtoo <- list()  #Name of the files are stored in a list: name_wt

for (i in (1:length(file_names))) {
  brain_ls_youtoo[[i]] <- file_names[i] %>%
  read_h5()
  name_youtoo[i] <- sub("_Probabilities.h5", "", file_names[i])
}

##Test to see if data are correct
# tidy_brain_ls_youtoo <- list()
# 
# tidy_brain_ls_youtoo[[1]] <- file_names[1] %>%
#   read_h5()%>%
#   tidy()
# mean(tidy_brain_ls_youtoo[[1]]$x)  #39.35473
# 
# tidy_brain_ls_youtoo[[2]] <- file_names[2] %>%
#   read_h5()%>%
#   tidy()
# mean(tidy_brain_ls_youtoo[[2]]$x)  #24.65265
...
#wildtype
qmodel_sum_tb_wt<- data.frame()
m <- list()
for (i in (1:length(brain_ls_wt))) {
  m<- qmodel.brain(brain_ls_wt[[i]])%>%
    mutate(sample = name_wt[[i]])
  qmodel_sum_tb_wt <- rbind(qmodel_sum_tb_wt, m)
}
```
