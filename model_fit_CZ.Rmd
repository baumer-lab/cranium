...
plot2d_plane <- function(x, plane = c("x", "z"), show_max = FALSE, ...) {
  ...
  ggplot(gg_data, aes_string(x = plane[1], y = plane[2], color = plot_var), ...) +
    ...
    geom_smooth(method = "lm", formula = y ~ I(x^2) + x +z, color = "red") +
...
tidy.brain <- function(x, threshold, type =NULL, ...) {  #type=NULL
#   if (is.null(type)){
#     threshold = threshold  # If user did not specify type, use the threshold instead
#     res <- x %>%
#     as.data.frame.table() %>%
...
#Edit 2
tidy.brain <- function(x, threshold.n, type=NULL, ...) {
  if(is.null(type)){
    if(is.null(threshold.n)){
      threshold =0.9
    }else{threshold = threshold.n}
  }
  if (type== "wildtype"){
    threshold = 0.9
  }
  if (type=="youtoo"){
    ...
    threshold = threshold.n    # If user did not specify type, use the threshold instead
  } 
    res <- x %>%
    as.data.frame.table() %>%
    mutate(x = as.integer(Var1) * 0.13,
...
#Edit 3
tidy.brain <- function(x,  type, threshold.n, ...) {
  if(missing(type)){
    if(missing(threshold.n)){
      threshold =0.9
    }else{threshold = threshold.n}
  }
  if (type== "wildtype"){
...
tidy_youtoo_sample<-tidy(data, threshold.n=0.3, type == "wildtype") #Works

tidy_youtoo_sample<-tidy(data, type == "wildtype") #Not working
